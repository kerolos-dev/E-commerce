...
const carateSubCategory = catchError(async (req, res, next) => {
  ...
  // check  file  upload
  if (!req.file)
    return next(new AppError("subCategory  image  is  reqouierd "));
  // upload   image cloudinary
  const { public_id, secure_url } = await cloudier.uploader.upload(
    req.file.path,
    {
      folder: `${process.env.CLOUD_FOLDER_NAME}/sudCategory`,
    }
  );
...
const updateSubCategory = catchError(async (req, res, next) => {
  ...
  const category = await CategoryModel.findById(req.params.category);
  if (!category)
    return next(new AppError("category  is not  found !", { cause: 404 }));
  ...
  if (!subCategory)
    next(new AppError("subcategory  is not  found !", { cause: 404 }));
...
const getAllSubCategory = catchError(async (req, res, next) => {
  ...
  // multiple populate
  const subcategoryALL = await subCategoryModel
    .find()
    .populate([
